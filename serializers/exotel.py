...
#         try:
#             logger.debug(f"Hanging up Plivo call {self._call_id}")
...
#             self.wf.writeframes(payload)
#             # Input: Convert Plivo's 8kHz μ-law to PCM at pipeline input rate
#             deserialized_data = await ulaw_to_pcm(
#                 payload, self._plivo_sample_rate, self._plivo_sample_rate, self._resampler
#             )

#             audio_frame = InputAudioRawFrame(
#                 audio=deserialized_data, num_channels=1, sample_rate=self._plivo_sample_rate
#             )
#             return audio_frame
...
class ExotelFrameSerializer(FrameSerializer):
    """Serializer for Exotel Audio Streaming WebSocket protocol.
    ...
    When auto_hang_up is enabled (default), the serializer will automatically terminate
    the Exotel call when an EndFrame or CancelFrame is processed, but requires Exotel
    credentials to be provided.

    Attributes:
        _stream_id: The Exotel Stream ID.
        _call_id: The associated Exotel Call ID.
        _auth_id: Exotel auth ID for API access.
        _auth_token: Exotel authentication token for API access.
        _params: Configuration parameters.
        _exotel_sample_rate: Sample rate used by Exotel (typically 8kHz).
        _sample_rate: Input sample rate for the pipeline.
        _resampler: Audio resampler for format conversion.
    """
    ...
    def __init__(
        ...
        Args:
            stream_id: The Exotel Stream ID.
            call_id: The associated Exotel Call ID (optional, but required for auto hang-up).
            auth_id: Exotel auth ID (required for auto hang-up).
            auth_token: Exotel auth token (required for auto hang-up).
            params: Configuration parameters.
        """
        self._stream_id = stream_id
        self._call_id = call_id
    ...
    def _resample_audio(self, audio_data: bytes, from_rate: int, to_rate: int) -> bytes:
        ...
        resampled = np.interp(indices, np.arange(len(audio_array)), audio_array.astype(float))
    ...
    async def serialize(self, frame: Frame) -> str | bytes | None:
        ...
        if (
            ...
            # For Exotel: Convert to 16-bit PCM at 8kHz (no μ-law conversion needed)
            if frame.sample_rate != self._exotel_sample_rate:
                # Resample to Exotel's expected sample rate
                resampled_data = self._resample_audio(data, frame.sample_rate, self._exotel_sample_rate)
            else:
                resampled_data = data
                
            # Base64 encode the PCM data for Exotel
            payload = base64.b64encode(resampled_data).decode("utf-8")
            
            answer = {
                "event": "playAudio",
                "media": {
                    "contentType": "audio/pcm",  # Changed from audio/x-mulaw to audio/pcm
                    "sampleRate": self._exotel_sample_rate,
                    "payload": payload,
                },
                "streamId": self._stream_id,
            }
    ...
    async def _hang_up_call(self):
        try:
            logger.debug(f"Hanging up Exotel call {self._call_id}")
            # Create basic auth from auth_id and auth_token
            # auth = aiohttp.BasicAuth(self._auth_id, self._auth_token)
        except Exception as e:
            logger.exception(f"Failed to hang up Exotel call: {e}")
    ...
    async def deserialize(self, data: str | bytes) -> Frame | None:
        ...
        if message.get("event") == "media":
            ...
            # For Exotel: Payload is already PCM data, no μ-law conversion needed
            # Just use the data directly since it's already in the right format
            deserialized_data = payload

            audio_frame = InputAudioRawFrame(
                audio=deserialized_data, 
                num_channels=1, 
                sample_rate=self._exotel_sample_rate
            )
            return audio_frame
        elif message.get("event") == "dtmf":
            dtmf_data = message.get("dtmf", {})
